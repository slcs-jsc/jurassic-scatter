...
void collect_read_atm(char *dirname,
		      char *filename,
		      ctl_t *ctl,
		      atm_t *atm_dest,
		      atm_t *atm_src);
...
double read_chisq(const char *dirname);
...
int main(int argc, char *argv[]) {
  ...
  int chisq_id[NVAR], cnt=-1, dimid[2], dimid_chisq[1], ig, iw,
    k_full[NDMAX]={0}, k_id[NDMAX][NVAR], lat_id[NVAR], lon_id[NVAR],
    na[NDSMAX], ncid, nds_id, np_id, np_dimid, nds_dimid,
    press_full=0, press_id[NVAR], q_full[NGMAX]={0}, q_id[NGMAX][NVAR],
    temp_full=0, temp_id[NVAR], time_id[NVAR], z_id[NVAR];
    
  size_t i, ind_start[]={0,0}, ind_end[]={0,0}, ind_start_chisq[]={0},
    ind_end_chisq[]={0}, max_np, ndir=0;
  ...
  addvar(ncid, "np", "1", "data point index",
	 NC_INT, dimid, 0, &np_id, 1);
  
  /* Atmospheric data... */
  addvar(ncid, "time", "s", "time (seconds since 2000-01-01T00:00Z)",
	 NC_DOUBLE, dimid, 0, time_id, 2);
  addvar(ncid, "z", "km", "altitude",
	 NC_DOUBLE, dimid, 0, z_id, 2);
  addvar(ncid, "lon", "deg", "longitude",
	 NC_DOUBLE, dimid, 0, lon_id, 2); 
  addvar(ncid, "lat", "deg", "latitude",
	 NC_DOUBLE, dimid, 0, lat_id, 2);
  ...
  while(fscanf(dirlist, "%s", dir)!=EOF) {
    ...
    *max_np=GSL_MAX((size_t)atm.np, *max_np);
	...
	(atm.z[ip]>=ctl->rett_zmin && atm.z[ip]<=ctl->rett_zmax);
      for(ig=0; ig<ctl->ng; ig++)
	q_full[ig]=q_full[ig] ||
	  (atm.z[ip]>=ctl->retq_zmin[ig] && atm.z[ip]<=ctl->retq_zmax[ig]);
      for(iw=0; iw<ctl->nw; iw++)
	k_full[iw]=k_full[iw] ||
	  (atm.z[ip]>=ctl->retk_zmin[iw] && atm.z[ip]<=ctl->retk_zmax[iw]);
    }
  }
...
void writevar(int ncid,
  ...
  if(full) {
    ...
    /* Noise error... */
    NC(nc_put_vara_double(ncid, varid[2], ind_start, ind_end, noise));
